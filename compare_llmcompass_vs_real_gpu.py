"""
Compare LLMCompass simulation results with real GPU execution latency.

Reads:
  1. LLMCompass simulation CSV from change_core_atlas2_for_realGPUcomparison.py
  2. Real GPU benchmark CSV from benchmark_real_gpu.py
...
REAL_GPU_CSV = "./real_gpu_benchmark_results.csv"
REPORT_OUTPUT = "./comparison_report.md"
...
def parse_llmcompass_csv(filepath):
    ...
    with open(filepath, "r") as f:
        ...
        for parts in reader:
            ...
            # Format: name, area, per_layer_latency, total_latency, qkv, q_mul_k, a_mul_v, h_matmul0, h1_matmul1, h2_matmul2, softmax, layernorm, layernorm, gelu, allreduce, allreduce
            name = parts[0]
            # Extract batch size from name like "Batch1_Input8192_Output1"
            ...
            per_layer_latency = float(parts[2])  # in seconds
            total_latency = float(parts[3])

            # simulate_log breakdown (12 values)
            qkv = float(parts[4])
            ...
            # no_QK_AV_softmax excludes QxK, AxV, and softmax from the total.
...
def parse_real_gpu_csv(filepath):
    """Parse real GPU benchmark results CSV."""
    results = {}
    metadata = {
        "gpu_names": set(),
...
def generate_report(llmcompass, real_gpu, real_gpu_metadata):
    ...
    lines.append(f"**Mode:** Decode (auto-regression, seq_len=1 token)")
    lines.append(f"**Excluded operations:** QxK, Softmax, AxV (multi-head attention)")
    lines.append(f"**LLMCompass config:** H100 (GH100_80GB), heuristic-GPU, no_QK_AV_softmax target")
    ...
    op_labels = ["QKV Proj", "Output Proj (W0)", "FFN Up (W1)", "FFN Down (W2)", "LayerNorm", "GeLU"]

    for op_name, op_label in zip(op_names, op_labels):
        lines.append(f"### {op_label}")
        lines.append("")
        lines.append("| Batch Size | LLMCompass (us) | Real GPU (us) | Error (%) |")
        lines.append("|:----------:|:---------------:|:-------------:|:---------:|")
        for bs in batch_sizes:
            sim_val = llmcompass[bs][op_name]
            real_val = real_gpu[bs][op_name]
            if op_name == "layernorm":
                # LLMCompass reports one layernorm; real GPU also reports one
                pass
    ...
    lines.append("")
    lines.append("| Batch Size | Component | LLMCompass (us) | Real GPU (us) | Error (%) |")
    lines.append("|:----------:|:---------:|:---------------:|:-------------:|:---------:|")
    for bs in batch_sizes:
        s = llmcompass[bs]
        r = real_gpu[bs]
        for comp, label in [("matmul_total", "GEMM Total"), ("norm_total", "Norm Total"), ("gelu", "GeLU")]:
            sv = s[comp]
            rv = r[comp]
            err = pct_error(sv, rv)
    ...
    lines.append("")
    lines.append("| Batch Size | LLMCompass (ms) | Real GPU (ms) | Error (%) |")
    lines.append("|:----------:|:---------------:|:-------------:|:---------:|")
    for bs in batch_sizes:
    ...
    if "H100" not in gpu_name:
        lines.append("- Real GPU latency was not measured on H100 in this CSV. "
                      "Results will differ due to GPU architecture and should be re-run on H100 for a fair comparison.")
    lines.append("- The comparison total is `QKV + W0 + W1 + W2 + 2*LayerNorm + GeLU`; QxK, Softmax, and AxV are excluded from both sides.")
    lines.append(f"- Each operation was measured with {warmup_iters} warmup iterations and {measure_iters} measurement iterations (median).")
    lines.append("- LLMCompass still logs QxK/AxV/softmax values for audit, but those values are excluded from the comparison total.")
    if LEGACY_SOFTMAX_REMOVED in source_modes:
        lines.append("- The input LLMCompass CSV was generated by a legacy `no_QK_AV` run; this report removed the logged softmax latency from the comparison total.")
...
def main():
    ...
    if not os.path.exists(REAL_GPU_CSV):
        print(f"ERROR: Real GPU CSV not found at {REAL_GPU_CSV}")
        print("Run benchmark_real_gpu.py first.")
        return
