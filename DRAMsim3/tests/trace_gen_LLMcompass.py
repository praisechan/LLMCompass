...
class Generator():
    ...
    def __init__(self, stream_type, interarrival, ratio, gb, shift_bits, num_ch=1, ch_pos=-1, num_bank=1, bank_pos=-1, mem_type = None, ro_pos=-1):
        ...
        if stream_type == 'random':
            ...
            for i in range(num_bank):
                  self._last_rd_addr_arr[i] += (2**bank_pos) * i #bank_pos: index of which channel bit starts from.
            assert bank_pos != -1, 'bank_pos should be assigned for bank interleaving'
    ...
    def _readonly_bank_interleave_stream_coarse(self):       
        if self.bank_counter[self.last_bank] == 0:
            ...
            if self.mem_type=="LPDDR4":
              ...
              if self._last_rd_addr_arr[self.last_bank] % 2**self.bank_pos == 0:
                  self._last_rd_addr_arr[self.last_bank] -= 2**self.bank_pos
...
if __name__ == '__main__':
    ...
    if args.mem_type == "DDR4":
      g = Generator(stream_type, args.interarrival, args.ratio, args.gb, args.shift_bits, num_ch=2, ch_pos=17, num_bank=args.num_bank, bank_pos=13, mem_type="DDR4")#DDR4
    elif args.mem_type == "LPDDR4":
      g = Generator(stream_type, args.interarrival, args.ratio, args.gb, shift_bits=128, num_ch=args.num_ch, ch_pos=17, num_bank=args.num_bank, bank_pos=13, ro_pos=19, mem_type="LPDDR4")#LPDDR4_4ch
    elif args.mem_type == "LPDDR4_mobile":
      g = Generator(stream_type, args.interarrival, args.ratio, args.gb, shift_bits=32, num_ch=args.num_ch, ch_pos=15, num_bank=args.num_bank, bank_pos=11, ro_pos=17, mem_type="LPDDR4")#LPDDR4_4ch_mobile
    elif args.mem_type == "HBM2":
      # HBM' mapping is rorabgbachco; channel comes earlier than bank
      import math
      channel_bit = math.ceil(math.log2(args.num_ch))
      bank_pos = 13 + channel_bit
      ro_pos = 14 + channel_bit
      g = Generator(stream_type, args.interarrival, args.ratio, args.gb, shift_bits=64, num_ch=args.num_ch, ch_pos=11, num_bank= 4, bank_pos=bank_pos, ro_pos = ro_pos, mem_type="HBM2")#HBM2
    elif args.mem_type == "HBM3":
      # HBM' mapping is rorabgbachco; channel comes earlier than bank
      channel_bit = math.ceil(math.log2(args.num_ch))
      bank_pos = 13 + channel_bit
      ro_pos = 14 + channel_bit
      g = Generator(stream_type, args.interarrival, args.ratio, args.gb, shift_bits=64, num_ch=args.num_ch, ch_pos=11, num_bank= 4, bank_pos=bank_pos, ro_pos = ro_pos, mem_type="HBM3")#HBM3
    else:
