...
class Generator():
    ...
    def __init__(self, stream_type, interarrival, ratio, gb, shift_bits, num_ch=1, ch_pos=-1, num_bank=1, bank_pos=(-1,-1)):
        ...
        elif stream_type == 'readonly_interleaving_bank_sweep':
            self._last_rd_addr = 0
            self._gen = self._readonly_interleaving_bank_sweep
        else:
    ...
    # def _readonly_bank_interleave_stream(self):
    #     if self.last_bank==self.num_bank-1:
    #       self.last_bank = 0
    #     else:
    #       self.last_bank += 1

    #     #if address exceeds bank field, roll back to initial value
    #     if self._last_rd_addr_arr[self.last_bank] - (2**self.bank_pos[0]) * self.last_bank >= 2**self.bank_pos[0]: 
    #       self._last_rd_addr_arr[self.last_bank] = 2**self.bank_pos[0] * self.last_bank
    ...
    def _readonly_interleaving_bank_sweep(self): 
        # for testing different number of interleaving banks. Not for normal read.
        self._last_rd_addr += self.shift_bits

        #if address exceeds bank field, add higher bit above bank field(rank or something)
        if self._last_rd_addr % 2**self.bank_pos[0] == 0 and self._last_rd_addr % 2**self.bank_pos[1] == self.num_bank * 2**self.bank_pos[0]:
            self._last_rd_addr -= 2**self.bank_pos[0] * self.num_bank
            self._last_rd_addr += 2**self.bank_pos[1]
        
        return ('r', self._last_rd_addr)
        
    def _readonly_mult_ch_stream_gen(self):
        if self.last_ch==self.num_ch-1:
          self.last_ch = 0
        else:
          self.last_ch += 1

        #if address exceeds channel field, roll back to initial value        
        if self._last_rd_addr_arr[self.last_ch] - (2**self.ch_pos) * self.last_ch >= 2**self.ch_pos: #if address exceeds bank field
          self._last_rd_addr_arr[self.last_ch] = 2**self.ch_pos * self.last_ch
         
        self._last_rd_addr_arr[self.last_ch] += self.shift_bits
        return ('r', self._last_rd_addr_arr[self.last_ch])
...
if __name__ == '__main__':
    ...
    # g = Generator(stream_type, args.interarrival, args.ratio, args.gb, args.shift_bits, num_ch=2, ch_pos=17, num_bank=args.num_bank, bank_pos=(14, 18))#HBM2
    g = Generator(stream_type, args.interarrival, args.ratio, gb=1, shift_bits=128, num_ch=1, ch_pos=18, num_bank=args.num_bank, bank_pos=(14, 17))#LPDDR4
    # g = Generator(stream_type, args.interarrival, args.ratio, args.gb, args.shift_bits, num_ch=2, ch_pos=17, num_bank=args.num_bank, bank_pos=(13, 16))#DDR4
