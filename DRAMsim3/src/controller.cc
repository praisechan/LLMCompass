...
void Controller::ClockTick() {
    ...
    // record last read command finished cycle
    if (cmd_issued && cmd.cmd_type == CommandType::READ) {
        last_read_cmd_cycle_ = clk_;
    }
...
void Controller::PrintFinalStats() {
    // print finished cycle of last transaction
    float trans_finish_cycle = last_read_cmd_cycle_ + config_.read_delay;
