...
int main(int argc, const char **argv) {
    ...
    args::ValueFlag<std::string> output_dir_arg(
        ...
        {'o', "output-dir"}, ".");
    args::ValueFlag<std::string> output_file_arg(
        parser, "output_file", "Output file name for stats files",
        {'f', "output-file"}, ".");
    ...
    std::string output_file = args::get(output_file_arg);
    ...
    if (!trace_file.empty()) {
        cpu =
            new TraceBasedCPU(config_file, output_dir, output_file, trace_file);
    } else {
        if (stream_type == "stream" || stream_type == "s") {
            cpu = new StreamCPU(config_file, output_dir, output_file);
        } else {
            cpu = new RandomCPU(config_file, output_dir, output_file);
        }
    }
