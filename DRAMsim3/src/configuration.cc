...
DRAMProtocol Config::GetDRAMProtocol(std::string protocol_str) {
    std::map<std::string, DRAMProtocol> protocol_pairs = {
        ...
        {"HBM", DRAMProtocol::HBM},       {"HBM2", DRAMProtocol::HBM2},
...
NVMType Config::GetNVMType(std::string nvm_type) {
    std::map<std::string, NVMType> nvm_pairs = {
        {"volatile", NVMType::VM},
        {"non_volatile", NVMType::NVM},
    };

    if (nvm_pairs.find(nvm_type) == nvm_pairs.end()) {
        std::cout << "Unkwown/Unsupported NVM Type: " << nvm_type
                  << " Aborting!" << std::endl;
        AbruptExit(__FILE__, __LINE__);
    }
    return nvm_pairs[nvm_type];
}
...
void Config::InitDRAMParams() {
    ...
    nvm_type = GetNVMType(reader.Get("dram_structure", "nvm_type", "volatile"));
...
void Config::InitOtherParams() {
    ...
    output_prefix = output_dir + "/" + output_file;
    // output_prefix =
...
void Config::InitPowerParams() {
    ...
    if (nvm_type == NVMType::VM) {
        act_energy_inc =
            VDD * (IDD0 * tRC - (IDD3N * tRAS + IDD2N * tRP)) * devices;
    } else {
        act_energy_inc = VDD * (IDD0 * tRAS - (IDD3N * tRAS)) * devices;
    }
